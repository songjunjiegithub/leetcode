...
class WordDictionary {
    ...
    public void addWord(String word) {
        for (int i = 0; i < word.length(); i++) {
            if (i == this.list.size()) {
                this.list.add(new HashSet[26]);
                this.end.add(new HashSet<>());
                for (int j = 0; j < 26; j++) {
                    this.list.get(i)[j] = new HashSet<>();
                }
            }

            this.list.get(i)[word.charAt(i) - 'a'].add(id);
        }
        if (word.length() == this.list.size()) this.end.add(new HashSet<>());
        this.end.get(word.length()).add(id);
        this.id++;
    }

    public boolean search(String word) {
        if (word.length() > this.list.size()) return false;
        if (word.length() >= this.end.size()) return false;
        HashSet<Integer> result = new HashSet<>(this.end.get(word.length()));
        if (result.isEmpty()) return false;
        for (int i = word.length() - 1; i >= 0; i--) {
            if (word.charAt(i) != '.') {
                result.retainAll(this.list.get(i)[word.charAt(i) - 'a']);
                if (result.isEmpty()) return false;
            }
        }
...
//                flag = true;
//                if (result.isEmpty()) {
//                    result.addAll(this.list.get(i)[word.charAt(i) - 'a']);
//                } else {
//                    result.retainAll(this.list.get(i)[word.charAt(i) - 'a']);
//                }
//                if (result.isEmpty()) return false;
//            }
//        }
//        result.retainAll(this.end.get(word.length()));
//        return flag ? !result.isEmpty() : !this.end.get(word.length()).isEmpty();
    }
    ...
    public static String getRandomStr(int n) {
        ...
        for (int i = 0; i < n; i++) {
            stringBuilder.append((char) (Math.floor(Math.random() * 26) + 'a'));
        }
